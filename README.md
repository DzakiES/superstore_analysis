...
1. Cap discounts at ~20% on Tables and Bookcases, where deep discounts are the main cause of losses.
2. Review Binders' discounting/promo policy separately — high volume, small consistent losses, likely unmanaged discount creep rather than a strategic loss-leader.
3. Investigate whether Central's thin margin comes from product mix or a regional discounting habit, and align it with West/East practice.
4. Set a discount approval threshold around 20–25%, since that's the inflection point where the average line item flips from profit to loss.
5. Re-run this analysis quarterly to track whether discount discipline improves subcategory-level margins.

## Repo contents

| File | Description |
|---|---|
| `superstore_analysis.ipynb` | Full analysis: data cleaning checks, EDA, discount-vs-profit deep dive, regional breakdown, and recommendations |
| `orders.csv` | Source data (9,994 rows, 20 columns: order/customer/product IDs, sales, quantity, discount, profit, category/subcategory, dates, shipping, geography) |
...
This is the well-known "Sample Superstore" retail dataset, commonly used for BI/analytics practice (originally distributed with Tableau's sample workbooks).
